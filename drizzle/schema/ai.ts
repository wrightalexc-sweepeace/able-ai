...
export const userAiUsage = pgTable('user_ai_usage', {
  id: uuid('id').default(sql`gen_random_uuid()`).notNull().primaryKey(),
  userId: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }), // Link to the users table
...
export const escalatedIssues = pgTable('escalated_issues', {
  id: uuid('id').default(sql`gen_random_uuid()`).notNull().primaryKey(),
  userId: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }), // User who reported the issue
  ...
  adminUserId: uuid('admin_user_id').references(() => users.id, { onDelete: 'set null' }), // Admin assigned to the issue
