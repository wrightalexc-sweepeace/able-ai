...
firebase firebase apphosting:secrets:grantaccess --backend able uk-rtw-api-secret
echo $UK_RTW_COMPANY_NAME | firebase apphosting:secrets:set --force --data-file - uk-rtw-company-name
firebase firebase apphosting:secrets:grantaccess --backend able uk-rtw-company-name

# STRIPE
echo $NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY | firebase apphosting:secrets:set --force --data-file - stripe-publishable-key
firebase firebase apphosting:secrets:grantaccess --backend able stripe-publishable-key
echo $STRIPE_SECRET_KEY | firebase apphosting:secrets:set --force --data-file - stripe-secret-key
firebase firebase apphosting:secrets:grantaccess --backend able stripe-secret-key
echo $STRIPE_WEBHOOK_SIGNING_SECRET | firebase apphosting:secrets:set --force --data-file - stripe-webhook-signing-secret
firebase firebase apphosting:secrets:grantaccess --backend able stripe-webhook-signing-secret

# NILEDB
echo $NILEDB_URL | firebase apphosting:secrets:set --force --data-file - niledb-url
firebase firebase apphosting:secrets:grantaccess --backend able niledb-url
