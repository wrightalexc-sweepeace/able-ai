...
export const validateClientPassword = async (password: string) => {
    if (password.length < 10) {
        return { success: false, error: "Password should be at least 10 characters.", user: null };
    }
    ...
    if (!status.isValid) {
        ...
        if (status.meetsMaxPasswordLength) {
            return { success: false, error: "Password should be less than 64 characters.", user: null };
        }
        console.warn(`Password is invalid, checking policy: ${status.passwordPolicy}`);
...
export async function registerWithEmailPassword(email: string, password: string, name: string, phone: string | undefined): Promise<RegistrationResult> {
    try {
        ...
        return { success: false, error: err.message || "Failed to register.", user: null };
    }
} 