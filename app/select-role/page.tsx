...
import { useAppContext } from '@/app/hooks/useAppContext'; // Assuming this path
...
export default function SelectRolePage() {
  ...
  const { updateUserContext, isAuthenticated, isLoading: loadingAuth } = useAppContext();
...
const isViewQA = React.useMemo(() => {
  ...
  const handleRoleSelection = async (role: 'BUYER' | 'WORKER') => {
    if (!updateUserContext) {
      ...
      console.error("updateUserContext function is not available from useAppContext.");
      return;
    }
    ...
    <div className={styles.container}>
      <div className={styles.card}>
        ...
        <div className={styles.greeting}>
          <p className={styles.intro}>Hello, I'm Able, it's lovely to meet you.</p>
        ...
        <div className={styles.actions}>
          <ActionButton
            ...
            disabled={isLoading}
          >Hire a Gig Worker</ActionButton>
          <ActionButton
            bgColor="#41a1e8" // Darker blue for "Find Work"
            onClick={() => handleRoleSelection('WORKER')}
            disabled={isLoading}
          >Find Gig Work</ActionButton>
        </div>

        <p className={styles.note}>
          We are focusing on hospitality and events gigs right now... but add all
          your skills and watch this space as we enable gig work across industries.
        </p>
      </div>
    </div>
  );
