...
const UserMgmtPage: React.FC = () => {
  ...
  const [uiState, setUiState] = useState<'initial' | 'inputPassword' | 'success' | 'error'>('initial');
  ...
  const handleResetPassword = useCallback(async (currentActionCode: string) => {
    ...
    try {
      ...
      setMessage(`Resetting password for ${email}. Please enter your new password.`);
    } catch (err: unknown) {
      console.error('Error verifying password reset code:', err);
      if (err instanceof FirebaseError) {
        setError(err.message || 'Invalid or expired action code. Please try resetting your password again.');
      } else {
        setError('An unexpected error occurred while verifying the password reset code.');
      }
      setUiState('error');
    }
    setIsLoading(false);
  ...
  const submitNewPassword = async () => {
    ...
    try {
      ...
      setMessage('Password has been reset successfully! Attempting to sign you in...');
      try {
        if (!userEmail) {
          // This case should ideally not happen if verifyPasswordResetCode was successful
          setMessage('Password reset, but email not found for auto sign-in. Please sign in manually.');
        ...
        let signInErrorMessage = 'Password reset successfully, but auto sign-in failed. Please try signing in manually.';
      ...
      if (err instanceof FirebaseError) {
        setError(err.message || 'Failed to reset password. The code might have expired or the password is too weak.');
      } else {
        setError('An unexpected error occurred while confirming the password reset.');
      }
      setUiState('error');
    }
    setIsLoading(false);
  ...
  const handleRecoverEmail = useCallback(async (currentActionCode: string) => {
    ...
    try {
      ...
      setMessage(`Your email has been successfully reverted to ${oldEmail}. If you did not request this, please secure your account.`);
      setUiState('success');
      ...
      if (err instanceof FirebaseError) {
        setError(err.message || 'Invalid or expired action code. Unable to recover email.');
      } else {
  ...
  const handleVerifyEmail = useCallback(async (currentActionCode: string, currentContinueUrl: string | null) => {
    setIsLoading(true);
    setError(null);
    try {
      await applyActionCode(auth, currentActionCode);
      ...
      if (currentContinueUrl) {
        ...
        setMessage(prev => prev + ` You will be redirected shortly if a continue URL was provided, or click here: <a href='${currentContinueUrl}'>Continue</a>`);
      }
    } catch (err: unknown) {
      console.error('Error verifying email:', err);
      if (err instanceof FirebaseError) {
        setError(err.message || 'Invalid or expired action code. Please try verifying your email again.');
      } else {
  ...
  const handleSignIn = useCallback(async (currentActionCode: string, currentContinueUrl: string | null) => {
    setIsLoading(true);
    setError(null);
    try {
  ...
  }, [searchParams, handleResetPassword, handleRecoverEmail, handleVerifyEmail]);

  return (
    <div className={styles.container}>
      <div className={styles.card}>
        <h1 className={styles.title}>Account Management</h1>
      {isLoading && <p className={styles.loadingText}>Loading... {message}</p>}
      
      {!isLoading && uiState === 'initial' && message && <p className={styles.message}>{message}</p>}
      ...
      {uiState === 'inputPassword' && (
        <div>
          ...
          />
          <button onClick={submitNewPassword} className={styles.submitButton} disabled={isLoading}>
            {isLoading ? 'Saving...' : 'Save New Password'}
          </button>
        </div>
      )}

      {uiState === 'success' && <p className={styles.successMessage} dangerouslySetInnerHTML={{ __html: message }}></p>}
      {uiState === 'error' && error && <p className={styles.errorMessage}>Error: {error}</p>}
