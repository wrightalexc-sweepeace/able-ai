...
interface OriginalGigInfo { // For Able's initial message
  workerName: string;
  role: string;
  originalDate: string;
  originalLocation: string;
}
interface RehireWorkerData { // For the card
  workerId: string;
  name: string;
  avatarUrl?: string;
  role: string;
...
async function fetchRehireData(buyerUserId: string, gigId: string): Promise<{ originalGig: OriginalGigInfo, workerForRehire: RehireWorkerData } | null> {
  ...
  if (gigId === "pastGig123") {
    return {
      originalGig: {
        workerName: "Jerimiah Jones", role: "Bartender",
        originalDate: "last Tuesday", originalLocation: "Central Station",
      },
      workerForRehire: {
        workerId: "jerimiah-jones-id", name: "Jerimiah Jones", avatarUrl: "/images/avatar-jerimiah.jpg",
        role: "Bartender", ableGigs: 15, experienceYears: "3+",
        reviewKeywords: ["lively", "professional", "hardworking"],
        proposedHourlyRate: 15, proposedHours: 4, // Default from image, can be edited
        platformFeePercent: 6.5, paymentProviderFeeFixed: 0.20, paymentProviderFeePercent: 1.5
      }
    };
  }
  return null;
...
export default function RehirePage() {
  ...
  const [originalGigInfo, setOriginalGigInfo] = useState<OriginalGigInfo | null>(null);
  const [workerToRehire, setWorkerToRehire] = useState<RehireWorkerData | null>(null);
  const [isLoadingData, setIsLoadingData] = useState(true);
  const [isBooking, setIsBooking] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Chat messages state
  const [chatMessages, setChatMessages] = useState<Array<{ id: number, type: 'bot' | 'userAction', content: string | React.ReactNode }>>([]);

  // Auth check
  useEffect(() => {
    if (!loadingAuth) {
      if (!user?.isAuthenticated /*|| user?.uid !== pageBuyerUserId */ || !user?.canBeBuyer) {
        router.replace("/select-role");
      }
    }
  }, [user?.isAuthenticated, loadingAuth, user?.uid, pageBuyerUserId, user?.canBeBuyer, router]);
      ...
      fetchRehireData(user.uid, gigId)
        .then(data => {
          if (data) {
            ...
            setChatMessages([
              { id: 1, type: 'bot', content: `Awesome, you want to rehire ${data.originalGig.workerName} for a ${data.originalGig.role} shift, similar to your gig on ${data.originalGig.originalDate} at ${data.originalGig.originalLocation}? (Proposing a 4 hour shift for next Friday at 6pm - you can edit this.)`},
              { id: 2, type: 'bot', content: `Perfect, ${data.originalGig.workerName} is generally available. Click the card to edit details or book them now!`},
            ]);
          } else {
  ...
  const handleBookWorker = async () => {
    ...
    setIsBooking(true); setError(null);
    ...
    try {
      ...
      setChatMessages(prev => [...prev, {id: Date.now(), type: 'bot', content: `Great! ${workerToRehire.name} has been booked for the new gig. Details are in your calendar.`}]);
      // router.push(`/user/${newGig.id}`); // Navigate to new gig details
    } catch (err: any) {
      setError(err.message);
      setChatMessages(prev => [...prev, {id: Date.now(), type: 'bot', content: `Error: ${err.message}`}]);
    } finally {
      setIsBooking(false);
    }
  };
  ...
  if (loadingAuth || isLoadingData) {
    return <div className={styles.loadingContainer}><Loader2 className="animate-spin" size={32}/> Loading Rehire Information...</div>;
  }
  if (error) {
  ...
  if (!originalGigInfo || !workerToRehire) {
    return <div className={styles.container}><div className={styles.pageWrapper}><p className={styles.emptyState}>Information for rehire not available.</p></div></div>;
  }

  return (
    <div className={styles.container}>
      <ChatBotLayout ref={chatContainerRef}> {/* Pass ref if ChatBotLayout handles scrolling */}
        ...
        {error && !isBooking && <MessageBubble text={`Oops! ${error}`} senderType='bot' avatarSrc={BOT_AVATAR_SRC} />}
