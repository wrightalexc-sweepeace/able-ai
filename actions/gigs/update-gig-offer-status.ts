...
import { GigsTable, gigStatusEnum } from "@/lib/drizzle/schema";
...
export async function updateGigOfferStatus({ gigId, userId, role, action }: { gigId: string; userId: string; role: 'buyer' | 'worker'; action: 'accept' | 'cancel'; isViewQA?: boolean; }) {
  ...
  try {
    ...
    const updatedGig = await db.update(GigsTable)
      .set({ statusInternal: newStatus })
      .where(and(eq(GigsTable.id, gigId), eq(gigUserIdCondition, userId)));

    return { updatedGig, status: 200 };
